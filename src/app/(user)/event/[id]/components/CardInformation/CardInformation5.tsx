...
export default function CardInformation5({ event, editable }: Props) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      QRCode.toDataURL(url, { margin: 1, width: 64 }).then(setEventQrThumb).catch(console.error);
      QRCode.toDataURL(url, { margin: 1, width: 400 }).then(setEventQrLarge).catch(console.error);
    }
  }, [id]);
  ...
  useEffect(() => {
    const genQrs = async () => {
      ...
      for (const r of roles) {
        ...
        if (!token) continue;
        const link = `${origin}/event/${id}/invite?token=${encodeURIComponent(token)}`;
        try {
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="space-y-3 flex-1">
                ...
                {organizers.length > 3 && (
                  <div className="pl-4 text-xs font-medium text-(--role-organizer) flex items-center gap-1">
                    <div className="w-6 h-px bg-pink-200"></div>+ {organizers.length - 3}{" "}
                    {t("organizers.moreOrganizers")}
                  </div>
                )}
              </div>
            ) : (
                ...
                <div className="flex-1 space-y-2 min-w-0">
                  <div className="flex items-center gap-1">
                    <Button
                      ...
                      title={
                        typeof window !== "undefined" ? `${window.location.origin}/event/${id}` : ""
                      }
                    >
      ...
      <Dialog open={!!refreshRole} onOpenChange={(o) => !o && setRefreshRole(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("invite.refreshConfirmTitle")}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">{t("invite.refreshConfirmDesc")}</p>
      ...
      <Dialog open={organizerListOpen} onOpenChange={setOrganizerListOpen}>
        <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            {organizers.map((org) => (
              <div key={org.id} className="flex items-center gap-4 p-3 rounded-2xl bg-card border">
                <UserAvatar
                  user={org.user}
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <span className="text-xs text-muted-foreground">{org.user?.email || ""}</span>
                </div>
              </div>
            ))}
          </div>
