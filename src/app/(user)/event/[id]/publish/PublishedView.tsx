                      ...
                      (localEvent?.awardsUnused && localEvent.awardsUnused.length > 0) ? (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const initial =
                              localEvent?.specialRewards && localEvent.specialRewards.length > 0
                                ? localEvent.specialRewards.map((r: any) => ({
                                    ...r,
                                    pendingFile: undefined,
                                    preview: r.image ?? null,
                                  }))
                                : (localEvent?.awardsUnused ?? []).map((s: string, i: number) => ({
                                    id: `temp-${i}-${Date.now()}`,
                                    name: s,
                                    description: "",
                                    image: null,
                            ...
                            initial.forEach((r: any) => {
                              if (r.preview) previews[r.id] = r.preview;
                            });
                            setSrPreviews(previews);
                            setRemovedRewardIds([]);
                            setForm({ specialRewards: initial.map((r: any) => r.name) });
                          }}
                        >
                          <Edit3 className="h-4 w-4" /> แก้ไข
                        </Button>
