...
export default function DashboardPage() {
  ...
  const handleCreateEvent = async (eventName: string) => {
    try {
      ...
      if (typeof err === "object" && err) {
        const backendMessage = (err as AxiosError<{ message: string }>)?.response?.data?.message as string | undefined;
        if (backendMessage) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/40 flex justify-center px-4 py-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="flex flex-col sm:flex-row gap-3 items-stretch">
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <Button size="sm" variant={filter === "DRAFT" ? "default" : "outline"} onClick={() => setFilter("DRAFT")}>Draft</Button>
                  <Button size="sm" variant={filter === "LIVE" ? "default" : "outline"} onClick={() => setFilter("LIVE")}>Live</Button>
                          ...
                          <Card key={event.id} className="overflow-hidden hover:shadow-md transition-shadow">
                            <div className="grid grid-cols-[144px_1fr_auto] gap-4 items-center p-4">
                              ...
                              <div className="space-y-1">
                                <h4 className="font-semibold line-clamp-1">{event.eventName}</h4>
                                <p className="text-xs text-muted-foreground">
                                  {event.role}{event.isLeader ? " • Leader" : ""}
                                </p>
                                <p className="text-xs text-muted-foreground">
