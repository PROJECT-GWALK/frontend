...
export default function DashboardPage() {
  ...
  const getFilterTooltip = (filter: FilterType) => {
    const tooltips = {
      all: t.homePage.filterTooltip?.all || "Show all your events",
      draft: t.homePage.filterTooltip?.draft || "Events that haven't been published yet",
      upcomingRecruit:
  ...
  const [roleFilter, setRoleFilter] = useState<
    "ALL" | "ORGANIZER" | "PRESENTER" | "COMMITTEE" | "GUEST"
  >("ALL");
            ...
            <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>{t.homePage.createDialog.description}</DialogDescription>
                </DialogHeader>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredEvents.map((event) => {
                    ...
                    const roleColor = !event.isDraft
                      ? getRoleColorVar(event.role ?? undefined)
                      : undefined;
                          ...
                          <div className="space-y-2">
                            ...
                            {event.isDraft ? (
                              ...
                              <div className="space-y-2 pt-1">
                                {(() => {
                                  const subStart = event.startJoinDate
                                    ? new Date(event.startJoinDate)
                                    : null;
                                  const subEnd = event.endJoinDate
                                    ? new Date(event.endJoinDate)
                                    : null;
                                  const eventStart = event.startView
                            ...
                            {!event.isDraft &&
                              ...
                              event.role !== "ORGANIZER" && (
                                ...
                                >
                                  ...
                                  >
                                    <Star
                                      className={`mr-1 h-3 w-3 ${event.userRating ? "fill-yellow-500 text-yellow-500" : "text-primary"}`}
                                    />
                                    {event.userRating
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t.homePage.deleteDialog.description.replace(
                "{eventName}",
